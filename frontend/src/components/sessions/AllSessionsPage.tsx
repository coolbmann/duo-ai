...
export function AllSessionsPage() {
  ...
  const [tab, setTab] = useState<'all' | 'completed' | 'cancelled' | 'failed'>('all')
  ...
  return (
    <div className="h-full overflow-y-auto bg-bg-app">
      <div className="max-w-[1280px] mx-auto" style={{ padding: '36px 44px 80px' }}>
        <div className="mb-7">
          <h1 className="text-[26px] font-semibold tracking-tight text-text-dark m-0 mb-1.5">All Sessions</h1>
          <p className="text-[14px] text-text-mid m-0">
            Every conversation you've had with Duo. Open any session to replay the steps, audit decisions, or fork a new run from it.
          </p>
        </div>
        ...
        <div className="flex gap-1 border-b border-border-light mb-[22px]">
          {STATUS_TABS.map(([k, label]) => (
            ...
            >
              {label}
              <span className="ml-1.5 text-text-light font-normal">{counts[k as keyof typeof counts]}</span>
            </button>
          ))}
        </div>
        ...
        <div className="bg-bg-card border border-border-light rounded-xl overflow-hidden">
          <div
            ...
            style={{ gridTemplateColumns: 'minmax(220px, 2.4fr) 1.4fr 140px', padding: '12px 18px' }}
          >
            <div>Session</div>
            <div>Agent</div>
            <div>When</div>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((s) => (
              <button
                ...
                style={{ gridTemplateColumns: 'minmax(220px, 2.4fr) 1.4fr 140px', padding: '14px 18px' }}
              >
                <div className="flex flex-col gap-0.5 min-w-0">
                  <div className="text-[13.5px] font-medium text-text-dark truncate">{s.title}</div>
                  <div className="text-text-light tracking-[0.04em]" style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5 }}>
                    {s.id}
                  </div>
                </div>
                <div>
                  <span className="inline-flex items-center gap-1.5 text-[12px] text-text-mid bg-bg-app border border-border-light rounded-full" style={{ padding: '3px 10px 3px 6px' }}>
                    <span className="w-[18px] h-[18px] rounded-[5px] bg-[#EDFBC8] inline-flex items-center justify-center text-[11px]">🎾</span>
                    {s.agent}
                  </span>
                </div>
