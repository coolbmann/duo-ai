  ...
  return (
    <div className="bg-bg-app border border-border-light rounded-[10px] p-[14px_16px]">
      ...
      <div
        ...
        style={{ fontFamily: 'var(--font-mono)', fontSize: 11, maxHeight: 240, overflowY: 'auto' }}
      >
        {lines.length === 0 ? (
          <div className="text-text-light text-[11px]">Idle. Start a chat to see agents work.</div>
        ) : (
          lines.map((l, i) => {
            const marker = l.status === 'done' ? '✓' : l.status === 'error' ? '✗' : l.status === 'active' ? '▶' : '○'
            return (
              <div
                key={i}
              ...
              >
                <span
                  className="text-[10px]"
                  style={{ color: l.status === 'done' ? '#3DDB7A' : l.status === 'active' ? '#2D6FF5' : '#8A8A85' }}
                >
                  {marker}
                </span>
                <div className="flex flex-col gap-0.5">
                  <span className="text-[9.5px] font-semibold uppercase tracking-[0.06em]" style={{ color: '#2DD4BF', fontFamily: 'var(--font-sans)' }}>
                    {l.agent}
                  </span>
                  <span className="text-[11px] text-text-mid truncate" style={{ color: l.status === 'done' ? '#1A1A1A' : undefined }}>
                    {l.msg}
                  </span>
                </div>
...
const STEPS = [
  ...
  { id: 'select', label: 'Awaiting selection' },
  { id: 'coordinate', label: 'Booking coordination' },
  { id: 'notify', label: 'Group notification' },
  { id: 'done', label: 'Done' },
]
const ORDER: Stage[] = ['idle', 'intent', 'discover', 'select', 'coordinate', 'notify', 'done']
  ...
  return (
    <div className="bg-bg-app border border-border-light rounded-[10px] p-[14px_16px]">
      ...
      <div className="flex flex-col gap-2.5">
        {STEPS.map((s, i) => {
          const stepIdx = i + 1
          const status = stepIdx < idx ? 'done' : stepIdx === idx ? 'active' : 'pending'
          ...
          return (
            ...
            >
              <span
                ...
                style={{
                  color: status === 'done' ? '#3DDB7A' : status === 'active' ? '#2D6FF5' : '#D6D6D2',
  ...
  return (
    <div className="bg-bg-app border border-border-light rounded-[10px] p-[14px_16px]">
      ...
      <div className="grid gap-y-2" style={{ gridTemplateColumns: '90px 1fr' }}>
        {entries.map(({ k, v }) => (
          <>
            <div key={k + '-k'} className="text-[11px] uppercase tracking-[0.06em] text-text-light font-medium self-center">{k}</div>
            <div key={k + '-v'} className="text-[13px] text-text-dark font-medium">{v}</div>
          </>
        ))}
      </div>
    </div>
